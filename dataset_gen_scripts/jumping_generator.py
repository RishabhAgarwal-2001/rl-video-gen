...
# Speed of stick man
speed = -1
# Number of frames to generate. Stick man moves speed units every frame
number_of_frames = 8
...
def generate_frames (im, directory, number_of_frames, root, initial_leg_theta, delta_leg_theta, initial_hand_theta, delta_hand_theta, speed, face_radius, body_length, hand_length, leg_length, test=False):
    ...
    theta_leg = initial_leg_theta

    pos = root.copy()

    # Generates frames
    for i in range (0, number_of_frames):
        draw_body(im, pos, body_length, face_radius, hand_length, theta_hand, leg_length, theta_leg)
        filename = os.path.join(folder_name, str(i)+'.jpg')
        cv2.imwrite(filename, im*255)
        erase_body(im, pos, body_length, face_radius, hand_length, theta_hand, leg_length, theta_leg)

        theta_hand += delta_hand_theta
        theta_leg += delta_leg_theta

        # Hand angle
        if (theta_hand > max_hand_theta):
            delta_hand_theta = -delta_hand_theta
            theta_hand = max_hand_theta - abs(theta_hand) + max_hand_theta
        elif (theta_hand < initial_hand_theta):
            delta_hand_theta = -delta_hand_theta
            theta_hand = initial_hand_theta - abs(theta_hand) + initial_hand_theta

        # Leg angle
        difference = abs(theta_leg) - min_leg_theta
        if (theta_leg < min_leg_theta):
            delta_leg_theta = -delta_leg_theta
            theta_leg = min_leg_theta - abs(theta_leg) + min_leg_theta
        elif (theta_leg > initial_leg_theta):
            delta_leg_theta = -delta_leg_theta
            theta_leg = initial_leg_theta - abs(theta_leg) + initial_leg_theta
            ...
            for body_length in body_lengths:
                for hand_length in hand_lengths:
                    for leg_length in leg_lengths:
                        generate_frames(im, directory, number_of_frames, root, initial_leg_theta, delta_leg_theta, initial_hand_theta, delta_hand_theta, speed, face_radius, body_length, hand_length, leg_length)
